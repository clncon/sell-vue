  ...
  export default {
    ...
    methods: {
      ...
      toggleContent(event) {
        ...
        this.$root.$emit('ratingtype.onlyContent', this.onlyContent);
      }
    },
    watch: {
      selectType(type) {
        this.selectTyped = type;
        this.onlyContented = type;
      },
      onlyContent(val) {
        this.onlyContented = val;
      },
      selectTyped(type) {
        this.$emit('on-type-select', type);
      },
      onlyContented(val) {
        this.$emit('on-onlyContent-select', val);
      }
    }
  };
</script>
